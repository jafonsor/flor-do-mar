...
- **One reload per ship**, shared by both broadsides, decremented as it is today. Disengaging clears permission and prevents a volley on the tick the reload completes; it does not cancel the reload. Re-engaging while the reload counter is above zero is rejected.
