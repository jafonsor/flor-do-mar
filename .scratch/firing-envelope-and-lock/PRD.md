...
- **Enemy behaviour stays dumb for this pass.** It locks the player once (after a short configurable delay from scenario start, not on tick 0), orders fire at will, and keeps orbiting its centre. It never disengages. No range-keeping or positioning logic. The delay is the named `enemyLockDelayTicks`, settled at **three ticks** in issue 05; it is a feel value that no play-test has confirmed, and issue 07 tunes it by watching the fight.
...
- **Hit-testing uses a screen-space radius** around each ship centre, computed from the existing screen-to-battle conversion, so hulls stay clickable as zoom changes. **The radius is one number**: the click hit test, the hover ring and the locked ring are all `reticleRadiusPixels`, a screen-space constant of 24 pixels converted once through the same camera and canvas size. The point of the design is that the hover ring appears exactly where the click will land, so if the number ever changes, all three change together — a hull-sized ring hit-tested at a different radius would make the marker lie about the gesture.
- **The reload circle animates smoothly; the wedges step per tick.** The circle lives in the DOM and needs no frame loop. Its fill *is* reload progress — zero at the volley, full when the guns are loaded — and the DOM interpolates it with a CSS transition one tick long, so the smoothness is the browser's and no per-frame clock is introduced. Disengaging mid-reload does not reset it: the guns reload whether or not they are permitted to fire, so the sweep runs on to full. None of the canvas animation machinery changes.
- **One shared reload fill**, drawn identically on both of a ship's wedges, matching the one shared cooldown.
- **A wedge's boundary is sampled once and shared.** A wedge is one render node per side whose geometry is built from a single sector sampler: the outline strokes the sampled arc at the envelope's full range, the fill fans that same arc at the radius the shared reload has reached, and the two are drawn as one shape in one colour. `RingStroke` hardcodes its angles from zero, so a partial arc needs a sampler; sampling it per channel would let the filled part and the outlined part be two different shapes. A test reads the drawn geometry out of the scene, puts it through the node's matrix, and asks the domain's own `broadsideGeometry` about the points on it, so the drawn envelope and the enforced one are proven to be the same area.
- **Two reticles, same size, different colour**: a hover circle on a lockable ship and a ring on the locked ship, drawn at the one reticle radius and above the hull in z. The hover ring is not drawn on the ship already locked — a click there releases the lock, so that ship wears the locked ring. **The hover is re-derived against every snapshot**, not only when the pointer moves: a ship moves under a pointer that is holding still, and a ring left behind on a hull the pointer has passed lies about the click that follows, which hit-tests the press afresh. No dashed line to the target in this pass.
...
- **First visual verification — moved from issue 03 to issue 07**: reduce the range and the zoom, sail the player into the enemy's reach, and confirm the enemy's envelope appears and the numbers are readable at 0.6. This is the riskiest assumption in the whole change — that a 96-unit envelope and a 16-unit hull are legible on a 760 by 428 canvas at once — and every visual decision downstream depends on it. Issue 03 could not run it: the wedge it is about only exists from issue 07, and driving a browser needs an escalation. Issue 03 checked the assumption numerically instead, and the numbers are recorded in its own file: at 0.6 the big boat renders 45.6 by 17.1 pixels and the small boat 25.65 by 8.55. That check is also why issue 03 did not zoom out further to bring the outward-facing wedges into view.
